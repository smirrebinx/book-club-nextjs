...
- **TypeScript:** 5.x
- **Database:** MongoDB with Mongoose 8.19.2 or later
- **Authentication:** NextAuth.js 5.0.0-beta.30 or later
- **Styling:** Tailwind CSS 4.1.16 or later with CSS Variables
- **Validation:** Zod 4.1.12 or later
- **Deployment:** Vercel
...
### Package Manager
- **npm** - Always use `npm ci` for installing dependencies (not `npm install`)
- `npm ci` ensures reproducible builds using package-lock.json
- Only use `npm install` when intentionally adding/updating packages
...
- Use ESLint directly via the configured script
...
This executes: `eslint . --config eslint.config.mjs`
...
- Maximum function length: 300 lines

**Minimum Package Versions:**
- eslint: 9.39.1 or later
- eslint-config-next: 16.2.2 or later (must match Next.js major version)
- @typescript-eslint/eslint-plugin: 8.46.2 or later
- @typescript-eslint/parser: 8.46.2 or later

**Important:** When updating Next.js, ensure eslint-config-next matches the same major version.
...
- Install command: `npm ci` (automatic on Vercel)
- Framework preset: Next.js

### Environment Variables on Vercel

Set all environment variables from `.env.local` in Vercel dashboard under:
...
- `ADMIN_EMAIL` (optional, for initial setup only)
...
4. Production deployment on main branch only
...
**IMPORTANT:** When installing dependencies in this project, always use:
```bash
npm ci
```
...
- Only when intentionally adding new packages: `npm install <package-name>`
- Only when intentionally updating packages: `npm install <package-name>@latest`

**Never use:**
- `npm install` without arguments (use `npm ci` instead)
- `npm install` in CI/CD pipelines (use `npm ci` instead)
- `npm install` after pulling changes (use `npm ci` instead)

### Commands Reference

| Command | Purpose | When to Use |
|---------|---------|-------------|
| `npm ci` | Clean install from lock file | After git pull, in CI/CD, general development |
| `npm install <pkg>` | Add new package | Adding new dependency |
| `npm install <pkg>@latest` | Update specific package | Updating a package |
| `npm run dev` | Start dev server | Development |
| `npm run build` | Production build | Testing build, CI/CD |
| `npm run lint` | Run ESLint | Before commits |
